...
public class AveragingHeuristic {
   ...
   private int moveOne() {
       ...
       if (currentLeafCountMap.get(poorestPartitionID) < this.thresholdLeafCount &&
           currentLeafCountMap.get(richestPartitionID) > this.thresholdLeafCount    ) {
               ...
               //logger.debug( "threshold is " +thresholdLeafCount + " pooerest "+currentLeafCountMap.get(poorestPartitionID) + " richest "+currentLeafCountMap.get(richestPartitionID) );
               //move 1 node
   ...
   private void  findThreshold () {
       ...
       thresholdLeafCount = -ONE + (long) Math.ceil(count/SparkSimulatorDriver.NUM_PARTITIONS);
   }
}
